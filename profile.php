...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 動的に振込先カードを挿入
    const container = document.querySelector('section .max-w-4xl');
    if (!container) return;

    const card = document.createElement('div');
    card.className = 'mt-6';
    card.innerHTML = `
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
            <h2 class="text-xl font-semibold text-gray-900">振込先情報</h2>
            <p class="text-sm text-gray-600">納品後に依頼者へ提示されます。本人のみ編集できます。</p>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700">銀行名</label>
                    <input type="text" id="bank_name" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="例）みずほ銀行">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700">支店名</label>
                    <input type="text" id="branch_name" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="例）渋谷支店">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700">口座種別</label>
                    <select id="account_type" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                ...
                <div>
                    ...
                    <input type="text" id="account_number" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="半角数字（ハイフン可）">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700">口座名義（カナ推奨）</label>
                    <input type="text" id="account_holder_name" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="例）ヤマダ タロウ">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700">口座名義カナ（任意）</label>
                    <input type="text" id="account_holder_kana" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="全角カナ">
                </div>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700">備考（任意）</label>
                <input type="text" id="note" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="振込時の注意事項など">
            </div>

            <div class="flex justify-end">
                <button type="button" id="btn-save-bank" class="px-6 py-2 rounded-md text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none">振込先を保存</button>
            </div>
        </div>
    `;
    ...
    (async () => {
        try {
            const res = await fetch('./api/get-bank-account.php?mode=self', { method: 'GET' });
            if (!res.ok) return;
            const json = await res.json();
            if (json && json.success && json.account) {
                const a = json.account;
                document.getElementById('bank_name').value = a.bank_name || '';
                document.getElementById('branch_name').value = a.branch_name || '';
                document.getElementById('account_type').value = a.account_type || '普通';
                document.getElementById('account_number').value = a.account_number || '';
                document.getElementById('account_holder_name').value = a.account_holder_name || '';
                document.getElementById('account_holder_kana').value = a.account_holder_kana || '';
    ...
    btn?.addEventListener('click', async function() {
        try {
            ...
            fd.append('branch_name', document.getElementById('branch_name').value.trim());
            fd.append('account_type', document.getElementById('account_type').value);
            fd.append('account_number', document.getElementById('account_number').value.trim());
            fd.append('account_holder_name', document.getElementById('account_holder_name').value.trim());
            fd.append('account_holder_kana', document.getElementById('account_holder_kana').value.trim());
            fd.append('note', document.getElementById('note').value.trim());

            const res = await fetch('./api/save-bank-account.php', { method: 'POST', body: fd });
            const json = await res.json();
            if (json && json.success) {
                if (typeof showNotification === 'function') {
                    showNotification(json.message || '保存しました', 'success');
                } else {
                    alert(json.message || '保存しました');
                }
            } else {
                const msg = (json && (json.errors?.join('\n') || json.error || json.message)) || '保存に失敗しました';
                if (typeof showNotification === 'function') {
